...
## Parser + Position info fix (from: https://bitbucket.org/plas/thonny/src/master/thonny/ast_utils.py)
...
def _range_contains_smaller_eq(self_lineno, self_col_offset, self_end_lineno, self_end_col_offset,
                               other_lineno, other_col_offset, other_end_lineno, other_end_col_offset):
    return (_range_eq(self_lineno, self_col_offset, self_end_lineno, self_end_col_offset, other_lineno,
                      other_col_offset, other_end_lineno, other_end_col_offset)
            or _range_contains_smaller(self_lineno, self_col_offset, self_end_lineno, self_end_col_offset, other_lineno,
                                       other_col_offset, other_end_lineno, other_end_col_offset))
...
def jsonify_ast(node, sourceBytes, level=0):
    fields = {}

    mark_text_ranges(node, sourceBytes)
    for k in node._fields:
        fields[k] = '...'
        v = getattr(node, k)
        if isinstance(v, ast.AST):
            if v._fields:
                fields[k] = jsonify_ast(v, sourceBytes)
            else:
            ...
            fields[k] = []
            for e in v:
                fields[k].append(jsonify_ast(e, sourceBytes))
    ...
    if hasattr(node, 'col_offset') and hasattr(node, 'lineno') and hasattr(node, 'end_col_offset') and hasattr(node, 'end_lineno'):
        range = {'start': {"col": node.col_offset, "row": node.lineno}, 'end': {"col": node.end_col_offset, "row": node.end_lineno}}
    else :
        range = None

    ret = { "type": classname(node), "range": range, "properties": fields }
    return ret

def make_ast(code):
    tree = ast.parse(code)
    my_str_as_bytes = str.encode(code)
    return json.dumps(jsonify_ast(tree, my_str_as_bytes)) + u"\u0004"
...
def f(line): return make_ast(line)
